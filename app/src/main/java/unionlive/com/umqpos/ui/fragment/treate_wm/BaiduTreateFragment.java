...
import static unionlive.com.umqpos.content.Fields.DAY_FLAG_BAIDU;

/**
 * @author chenyu   Email:981214993@qq.com
 * @version 2016/12/15 13:59
 * @describe 这是-->外卖-->已处理-->百度的界面
 */
public class BaiduTreateFragment extends BaseFragment implements ItemClickListener, View.OnClickListener {
    ...
    private void getDatas() {
        mListStr = InPutJsonData.T201(getActivity(), Fields.loginsessionId, orderStatus,
                TitleInfomation.channelId_platform_baidu, PAGE_SIZE, pageIndex, TimeHelper.exceptDay(DAY_FLAG_BAIDU),
                TimeHelper.exceptDay(DAY_FLAG_BAIDU),"");//订单查询上送的密文
        NetUtil.waiMaiNetWorkUtil(mListStr, Fields.channelId,
                OrderTreateListResponse.class, getActivity(),
                Fields.ORDER_TYPE_TREATE_BAIDU);
    }
    ...
    private void initView(View view) {
        ...
        setWhichDay(Fields.DAY_FLAG_BAIDU);
    }
                ...
                if (dy > 0) {//如果是向下滑动
                    ...
                    if (lastVisibleItemPosition + 1 == mTreateAdapter.getItemCount()) {
                        ...
                        if (!isRefreshing && !isLoading && !isEnd) {
                            ...
                            pageIndex++;
                            getDatas();
                            //                        Toast.makeText(getActivity(), "pageIndex:" + pageIndex, Toast.LENGTH_SHORT).show();
                        }
                    }
                }
...
//                MyToast.show(getActivity(), "全部未处理界面---重新打印" + postion, true);
                ...
                NetUtil.waiMaiNetWorkUtil(enOrderDetialStr, Fields.channelId, DetialResponse.class,
                        getActivity(), Fields.ORDER_TYPE_TREATED_PRINT_BAIDU);//得到报文，通过EventBus获取
                break;
        }
    }
    ...
    public void onEvent(CoubonsDecoderEvent event) {
        if (event.getOrderType() == Fields.ORDER_TYPE_TREATE_BAIDU) {
            ...
            switch (treateListResponse.header.returnCode) {
                ...
                default:
                    ...
                    +":"+treateListResponse.header.returnCode);//上传错误日志
                    break;
            }

        }
        if (event.getOrderType() == Fields.ORDER_TYPE_TREATED_PRINT_BAIDU) {
