 ...
 */
public class ElemeTreateFragment extends BaseFragment implements ItemClickListener, View.OnClickListener {
    ...
    private void getDatas() {
        ...
        mTreateMsg = InPutJsonData.T201(getActivity(), Fields.loginsessionId, orderStatus,
                TitleInfomation.channelId_platform_eleme,
                PAGE_SIZE, pageIndex, TimeHelper.exceptDay(DAY_FLAG_ELEME),  TimeHelper.exceptDay(DAY_FLAG_ELEME),"");
        NetUtil.waiMaiNetWorkUtil(mTreateMsg, Fields.channelId,
                OrderTreateListResponse.class, getActivity(),
                Fields.ORDER_TYPE_TREATE_ELEME);
    ...
    private void initView(View view) {
        ...
        setWhichDay(Fields.DAY_FLAG_ELEME);
    }
    /**
     * 初始化选中哪一天
     * @param whichDay
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (dy > 0) {//如果是向下滑动
                    ...
                    if (lastVisibleItemPosition + 1 == mTreateAdapter.getItemCount()) {
                        ...
                        boolean isVertical = mRv_untreate_all.canScrollVertically(1);//判断是否滑动到底部
                        if (!isRefreshing && !isLoading && !isEnd&&isVertical) {
                            isLoading = true;
                            pageIndex++;
                            getDatas();
                        }
        ...
        switch (view.getId()) {
            ...
            case R.id.bt_wm_reprint:
                ...
                NetUtil.waiMaiNetWorkUtil(enOrderDetialStr, Fields.channelId, DetialResponse.class,
                        getActivity(), Fields.ORDER_TYPE_TREATED_PRINT_ELEME);//得到报文，通过EventBus获取
                break;
        }
    }
    ...
    public void onEvent(CoubonsDecoderEvent event) {
        if (event.getOrderType()==Fields.ORDER_TYPE_TREATE_ELEME) {
            ...
            switch (treateListResponse.header.returnCode) {
                ...
                default:
                    InitDataHelper.sendError(getActivity(), treateListResponse.header.returnMessage +
                            ":" + treateListResponse.header.returnCode);//上传错误信息
                    break;
            }

        }
        if (event.getOrderType() == Fields.ORDER_TYPE_TREATED_PRINT_ELEME) {//处理饿了么打印消息
...
//                    Printer landiPrinter = Printer.getInstance();
    ...
    public void onEvent(OrderStatuEvent event) {
        orderStatus = event.getStatus();//得到订单的状态--->>有populaUpwindow传来的
        initData();
    }
    @Subscribe(threadMode = ThreadMode.MAIN, sticky = true)
