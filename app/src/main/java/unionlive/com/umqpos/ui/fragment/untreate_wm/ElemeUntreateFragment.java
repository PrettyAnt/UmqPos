 ...
 * @describe 这是-->外卖-->未处理-->饿了么商家的界面
 */
public class ElemeUntreateFragment extends BaseFragment implements ItemClickListener {
    ...
    private void getDatas() {
        mListStr = InPutJsonData.T201(getActivity(), Fields.loginsessionId, -1,
                TitleInfomation.channelId_platform_eleme, PAGE_SIZE, pageIndex,
                TimeHelper.exceptDay(0), TimeHelper.exceptDay(0), "");//订单查询上送的密文 未处理只有今日
        NetUtil.waiMaiNetWorkUtil(mListStr, Fields.channelId, OrderUnTreateListResponse.class,
                getActivity(), Fields.ORDER_TYPE_UNTREATE_ELEME);
    }
            ...
            case R.id.bt_wm_reprint:
                ...
                if (TitleInfomation.printStatus == Printer.ERROR_NONE
                    ...
                    NetUtil.waiMaiNetWorkUtil(t301, Fields.channelId, OpenWmResponse.class, getActivity(),
                            Fields.ORDER_TYPE_ELEME_UNTREATE_RECEIVE);
                }
                break;
        }
    }
    ...
    public void onEvent(CoubonsDecoderEvent event) {
        if (event.getOrderType() == Fields.ORDER_TYPE_UNTREATE_ELEME) {
        ...
        //开始打印
        if (event.getOrderType() == Fields.ORDER_TYPE_UNTREATED_PRINT_ELEME) {//饿了么未处理打印
...
//                    Printer landiPrinter = Printer.getInstance();
        ...
        //确定接单的操作
        if (event.getOrderType() == Fields.ORDER_TYPE_ELEME_UNTREATE_RECEIVE) {
            ...
            switch (openWmResponse.header.returnCode) {
                case "0000"://成功接单
                    ...
                    NetUtil.waiMaiNetWorkUtil(t202, Fields.channelId, DetialResponse.class, getActivity(),
                            Fields.ORDER_TYPE_UNTREATED_PRINT_ELEME);
                    break;
                default:
                    FlushUi();
