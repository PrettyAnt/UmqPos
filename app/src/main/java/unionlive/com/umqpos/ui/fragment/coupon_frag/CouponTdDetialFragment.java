 ...
 * @describe ${TODO}
 */
public class CouponTdDetialFragment extends BaseFragment implements ItemClickListener {
...
//        EventBus.getDefault().postSticky(TodayDetialBean.class);
        ...
        mM202 = InPutJsonData.M202(getActivity(), transTime, hostTime, hostTrace, authCode,termId,shopId,shopOperId);
                        NetUtil.couponsNetWorkUtil(mM202, Fields.channelId, QueryTransDetailsResponse.class,
         getActivity(), Fields.QueryTransDetails,Fields.COUPON_TYPE_QUERY_TRANS_DETAILS_TD);//交易详情查询
        FragmentTransaction transaction = getActivity().getSupportFragmentManager()
                .beginTransaction();
        transaction.hide(this);
        TodayDetialFragment detialFragment = new TodayDetialFragment();
        transaction.add(R.id.fragment_counpon_content, detialFragment);
        transaction.commit();
        EventBus.getDefault().postSticky(new FlashUiEvent(detialFragment,this,true,"",Fields.FLASH_UI_TYPE_COUPON_TD));
    }
}
